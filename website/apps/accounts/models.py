...
class MyUser(AbstractEmailUser):
    '''
    User model based on AbstractEmailUser adding a unique slug (screenname)
    '''
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:  # new object -> slugify!
            #self.username = slugify(self.slug)[:50]
            self.name = self.slug
